...
namespace Coherence.Editor
{
    ...
    internal class CoherenceHeader
    {
        ...
        private string GetEmailUsername()
        {
            ...
            var indexOfAt = email.IndexOf("@", StringComparison.InvariantCulture);
            if (indexOfAt is -1)
            {
                return email;
            }

            var username = email.Remove(indexOfAt);
        ...
        private Vector2 PopupSize() => new(GetWidestTextWidth() + 35f, EditorGUIUtility.singleLineHeight * 3f + 10f);

        private void DrawLoginDetails()
        {
            DrawCenteredText(ProjectSettings.instance.Email);
            DrawCenteredText(GetOrganizationString());
            DrawCenteredText(GetProjectsString());
        }
        ...
        private string GetProjectsString()
        {
            ...
            if (projectCount is 1)
            {
                return $"Project: {projects[0].name}";
            }

            return $"Projects: {string.Join(", ", ProjectSettings.instance.GetValidAndDistinctProjects().Select(x => x.name))}";
        }

        private string GetOrganizationString() => $"Org: {PortalLoginDrawer.GetSelectedOrganization()?.name ?? "None"}";

        private float GetWidestTextWidth()
        {
            ...
            var projContent = new GUIContent(GetProjectsString());
