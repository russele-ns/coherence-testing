namespace Coherence.Editor
{
    ...
    public static class CoherenceSyncUtils
    {
        ...
        public static bool RemoveBinding(CoherenceSync sync, Component component, Descriptor descriptor)
        {
            ...
            if (binding is null)
            {
                var matchesByName = sync.Bindings.Where(b => b.Descriptor.Name == descriptor.Name).ToArray();
                if (matchesByName.Length is 1 && !matchesByName[0].UnityComponent)
                {
                    binding = matchesByName[0];
                }
            }
        ...
        /// <summary>
        /// Removes all the given <paramref name="bindings"/> from the <see cref="CoherenceSync">sync</see>.
        /// </summary>
        /// <param name="sync"> The <see cref="CoherenceSync"/> component to remove the bindings from. </param>
        /// <param name="bindings"> The bindings to remove. </param>
        /// <returns> The number of bindings that were removed. </returns>
        ...
        /// <summary>
        /// Gets the total number of variable bindings and the number of invalid variable bindings
        /// for a given <see cref="CoherenceSync"/> component and an <paramref name="context"/> GameObject.
        /// </summary>
        /// <param name="sync"> The <see cref="CoherenceSync"/> component to count variable bindings for. </param>
        /// <param name="context">
        /// <para>
        /// (Optional) GameObject whose components should be considered when counting variable bindings.
        /// </para>
        /// <para>
        /// The bindings of a <see cref="CoherenceSync"/> component can also include bindings targeting components
        /// attached to child GameObjects. If a <paramref name="context"/> parameter is provided, then only bindings
        /// targeting components attached to the given <paramref name="context"/> GameObject are included in
        /// the counts.
        /// If <paramref name="context"/> is <see langword="null"/>, then all bindings from the
        /// <see cref="CoherenceSync"/> component are included in the counts, regardless of the GameObject they target.
        /// </para>
        /// </param>
        /// <returns>
        /// A tuple containing the total number of variable bindings and the number of invalid variable bindings.
        /// </returns>
        internal static (int totalCount, int invalidCount) GetVariableBindingsCount(CoherenceSync sync, GameObject context = null)
        {
            var totalCount = 0;
            var invalidCount = 0;
            ...
            for (var index = 0; index < sync.Bindings.Count; index++)
            {
                ...
                if (context && binding.unityComponent && !ReferenceEquals(binding.unityComponent.gameObject, context))
                {
                    continue;
                }
        ...
        /// <summary>
        /// Gets the total number of command bindings and the number of invalid command bindings
        /// for a given <see cref="CoherenceSync"/> component and an <paramref name="context"/> GameObject.
        /// </summary>
        /// <param name="sync"> The <see cref="CoherenceSync"/> component to count command bindings for. </param>
        /// <param name="context">
        /// <para>
        ...
        internal static bool IsEditingBindingsAllowed(CoherenceSync sync, InteractionMode interactionMode)
            => IsEditingBindingsAllowed(new GameObjectStatus(sync.gameObject), interactionMode);

        internal static bool IsEditingBindingsAllowed(GameObjectStatus status, InteractionMode interactionMode)
        {
            if (status
                is { IsAsset: false }
        ...
        internal static int RemoveUnrecoverableBindings(CoherenceSync sync, Action onBeforeModifications = null)
        {
            if (!sync)
            {
                throw new ArgumentNullException(nameof(sync));
            ...
            var removedCount = sync.Bindings.RemoveAll(binding =>
            {
                if (IsBindingUnrecoverable(sync, binding))
                {
        ...
        /// <returns>Number of Bindings removed.</returns>
        /// <seealso cref="IsBindingValid(Coherence.Toolkit.CoherenceSync,int,out string)"/>
        public static int RemoveInvalidBindings(CoherenceSync sync) => RemoveInvalidBindings(sync, null);

        internal static int RemoveInvalidBindings(CoherenceSync sync, [MaybeNull] Func<Binding[], bool> confirm)
        {
            if (sync.Bindings.All(binding => IsBindingValid(sync, binding, out _)))
            {
                return 0;
            }

            var bindingsToRemove = sync.Bindings.Where(binding => !IsBindingValid(sync, binding, out _)).ToArray();
            if (confirm is not null && !confirm(bindingsToRemove))
            {
                return 0;
            }

            return RemoveBindings(sync, bindingsToRemove);
        }
        ...
        internal static bool UpdateBindings(CoherenceSync sync, bool forceUpdate) => EditorCache.UpdateBindings(sync, forceUpdate: forceUpdate);
