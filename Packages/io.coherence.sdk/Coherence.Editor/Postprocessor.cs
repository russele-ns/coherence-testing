...
namespace Coherence.Editor
{
    ...
    internal class Postprocessor : AssetPostprocessor
    {
        ...
        private static readonly LazyLogger logger = Log.GetLazyLogger<Postprocessor>();
        ...
        internal static void UpdateRuntimeSettings() => UpdateRuntimeSettings(SimulatorBuildPipeline.IsBuildingSimulator, null);
        internal static void UpdateRuntimeSettings(bool isBuildingSimulator, bool? isDevelopmentMode) => UpdateRuntimeSettings(RuntimeSettings.Instance, ProjectSettings.instance.MultipleProjects, isBuildingSimulator, isDevelopmentMode);

        internal static void UpdateRuntimeSettings(RuntimeSettings runtimeSettings, IReadOnlyList<ConditionalProject> multipleProjects, bool isBuildingSimulator, bool? isDevelopmentMode)
        {
            runtimeSettings.schemas = GetSchemaAssets();
            runtimeSettings.SchemaID = BakeUtil.SchemaID;

            if (!isBuildingSimulator)
            {
                multipleProjects.GetActive(isDevelopmentMode)?.ApplyToRuntimeSettings(runtimeSettings);
            }
        ...
        private static void TrySdkVersionUpdate()
        {
            ...
            if (codegen != runtimeSettings.RsVersion)
            {
                logger.Debug($"New engine version {runtimeSettings.RsVersion} detected.");
                EditorUserSettings.SetConfigValue(lastCodeGenKey, runtimeSettings.RsVersion);
            }
