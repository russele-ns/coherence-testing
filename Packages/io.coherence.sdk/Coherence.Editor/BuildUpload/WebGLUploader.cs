...
namespace Coherence.Editor
{
    ...
    {
        protected internal override string DialogTitle => "Batch upload?";
        protected internal override string GetMessage(string buildPath) => $"Contents in the following path will be uploaded one by one. Are you sure?\n\n{buildPath}";
        protected internal override string OkButton => "Batch upload";
        ...
        static bool GatherStreamingAssets(DirectoryInfo dir, string path, ICollection<FileToUpload> filesToUpload, [MaybeNull] ProjectInfo project)
        {
            if (!dir.Exists)
            {
                return true;
            ...
            {
                var uurl = UploadURL.GetWebGLFile(f.Length, Uri.EscapeDataString($"{path}{f.Name}"), true, project);
                if (uurl == null)
                {
                    return false;
                }
            ...
            {
                if (!GatherStreamingAssets(subDir, $"{path}{subDir.Name}/", filesToUpload, project))
                {
                    return false;
                }
            }
        ...
        static bool GatherGameAssets(DirectoryInfo dir, ICollection<FileToUpload> filesToUpload, [MaybeNull] ProjectInfo project)
        {
            FileInfo[] buildFiles = dir.GetFiles("*.*");
            var gameName = GetGameName(buildFiles);
            foreach (FileInfo f in buildFiles)
            {
                var uurl = UploadURL.GetWebGLFile(f.Length, f.Name.Replace(gameName, "game"), false, project);
                if (uurl == null)
                {
                    return false;
                }
        ...
        internal override bool Upload(AvailablePlatforms platform, string buildPath, [MaybeNull] ProjectInfo project)
        {
            ...
            if (!GatherGameAssets(buildDir, filesToUpload, project))
            {
                return false;
            }
            ...
            if (!GatherStreamingAssets(streamingDir, "", filesToUpload, project))
            {
                return false;
            }
            ...
            if (!UploadURL.RegisterBuild(platform: "webgl", filename: "", project))
            {
                return false;
            }
