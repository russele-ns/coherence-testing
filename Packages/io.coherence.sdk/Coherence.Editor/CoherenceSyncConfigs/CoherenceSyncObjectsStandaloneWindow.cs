...
namespace Coherence.Editor
{
    ...
    public class CoherenceSyncObjectsStandaloneWindow : EditorWindow
    {
        ...
        internal void OnGUI()
        {
            ...
            if (GUILayout.Button(GUIContents.refresh, ContentUtils.GUIStyles.toolbarButton))
            {
                treeView.Reload();
                GUIUtility.ExitGUI();
            }
            ...
            using (new EditorGUILayout.VerticalScope(CoherenceHubLayout.Styles.HorizontalMargins))
            {
                ...
                NetworkObjectsInfoDrawer.DrawRemoveInvalidBindingsFromRegistryButton(analyzer.AllEntriesInfo.InvalidBindings, OnRefresh);
                NetworkObjectsInfoDrawer.DrawMissingFromPreloadedAssets();
                if (CoherenceSyncConfigUtils.AnyIssues)
                {
                    if (GUILayout.Button("Delete Affected Configs", ContentUtils.GUIStyles.bigButton))
                    {
