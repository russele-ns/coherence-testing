    ...
    internal static class EditorHeaderDrawer
    {
        ...
        private static void DrawGameObjectGUI(Editor editor)
        {
            ...
            foreach (var target in editor.targets)
            {
                ...
                needsToConvertToPrefab |= (!PrefabUtility.IsPartOfAnyPrefab(gameObject) || !PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(gameObject)) &&
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                try
                {
                    ...
                    foreach (var target in editor.targets)
                    {
                        ...
                        if (create)
                        {
                            var prefabPath = needsToConvertToPrefab
                                ? $"Assets/{prefabAsset.name}.prefab"
