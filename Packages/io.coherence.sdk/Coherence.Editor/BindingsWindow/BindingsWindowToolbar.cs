...
namespace Coherence.Editor.Toolkit
{
    ...
    internal class BindingsWindowToolbar
    {
        ...
        private bool drawingSaveButton;
        ...
        internal void DrawFooter(BindingsWindow bindingsWindow)
        {
            ...
            if (bindingsWindow.ComponentIsPrefabAsset)
            {
                GUILayout.Space(5);

                var shouldDrawSaveButton = bindingsWindow.hasUnsavedChanges && !BindingsWindowSettings.AutoSave;
                if (drawingSaveButton != shouldDrawSaveButton)
                {
                    // Since the Save button is drawn using the layout system, we should only change whether its
                    // being drawn during layout events to avoid exceptions.
                    if (Event.current.type is EventType.Layout)
                    {
                        drawingSaveButton = shouldDrawSaveButton;
                    }
                    else
                    {
                        GUI.changed = true;
                ...
                if (drawingSaveButton && GUILayout.Button(saveContent, ContentUtils.GUIStyles.saveButton))
                {
