    ...
    internal partial class CoherenceSyncEditor : BaseEditor
    {
        ...
        private static partial class GUIContents
        {
            ...
            public static readonly GUIContent tagLabel = EditorGUIUtility.TrTextContent("Tag",
                "Adding a `tag` makes this object visible to any clients with a `TagQuery` using the corresponding tag. A tag can be made of a number of comma-delimited strings any of which are matched against a TagQuery's tag");
            ...
            public static readonly GUIContent notMappedPrefabMode = EditorGUIUtility.TrTextContent(
                    "CoherenceSync currently works only with prefabs.\nDrag this GameObject into a folder to create a prefab.");

            public static readonly GUIContent syncNeedsToBeApplied = EditorGUIUtility.TrTextContent(
                "CoherenceSync has to be applied to the prefab asset.");
            ...
            public const string InvalidInterpolationLocationMessage = "'Interpolation Location' must not be set to 'None' when a " + nameof(CoherenceNode) + " is attached.";
            public const string InvalidRigidbodyUpdateModeMessage = "'Rigidbody Update Mode' must be set to 'Direct' when a " + nameof(CoherenceNode) + " is attached.";

            public static class Channels
            {
                public static readonly GUIContent synchronizationChannel;

                static Channels()
                {
                    var supportedChannelsBuilder = new StringBuilder();
                    foreach (var channel in Channel.GetAll().Where(c => c.SupportsEntitySyncing))
                    {
                        _ = supportedChannelsBuilder.Append($"\n\n{channel.Name}: {channel.Description}");
                    }

                    var supportedChannels = supportedChannelsBuilder.ToString();
                    synchronizationChannel = new GUIContent("Synchronization Channel",
                        "Channel over which this entity will be synchronized. This includes creation, destruction, and binding synchronization but doesn't include commands." +
                        supportedChannels);
                }
            }
        }

        private static readonly LazyLogger logger = Log.GetLazyLogger<CoherenceSyncEditor>();
        ...
        private SerializedProperty synchronizationChannelProperty;
        ...
        protected override void OnEnable()
        {
            ...
            synchronizationChannelProperty = serializedObject.FindProperty(nameof(CoherenceSync.synchronizationChannel));
        ...
        private void DrawInstanceOverrideMode()
        {
            _ = EditorGUILayout.BeginVertical(GUI.skin.box);
            EditorGUILayout.LabelField(GUIContents.syncNeedsToBeApplied, ContentUtils.GUIStyles.centeredGreyMiniLabelWrap,
                GUILayout.MinHeight(60));
            if (GUILayout.Button("Apply to Prefab Asset", ContentUtils.GUIStyles.bigButton))
            {
                var path = PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(sync);
                PrefabUtility.ApplyAddedComponent(sync, path, InteractionMode.UserAction);
                GUIUtility.ExitGUI();
            }
            EditorGUILayout.EndVertical();
        }
        ...
        protected override void OnGUI()
        {
            ...
            {
                if (gameObjectStatus.SyncIsAddedOverride)
                {
                    CoherenceSyncValidator.DrawIssueHelpBoxes(sync, serializedObject, gameObjectStatus);
                    DrawInstanceOverrideMode();
                    return;
                }
        ...
        {
            if (CoherenceSyncConfigUtils.GetBindingInfo(sync) is not { } entryInfo)
            {
                return;
            }
            ...
            NetworkObjectsInfoDrawer.DrawRemoveInvalidBindingsButton(sync, gameObjectStatus, entryInfo.InvalidBindings, OnPerformedAction);

            if (IsEditingBindingsAllowed())
            {
        ...
        private bool IsEditingBindingsAllowed()
            => CoherenceSyncUtils.IsEditingBindingsAllowed(gameObjectStatus, InteractionMode.UserAction)
            && gameObjectStatus.PrefabInstanceStatus is not PrefabInstanceStatus.MissingAsset;
        ...
        private bool ShouldDisableSection(Editability editability = Editability.Default)
        {
            ...
            if (!editability.HasFlag(Editability.NonRootPrefabs) && gameObjectStatus is { IsRootOfAssetHierarchy: false, IsInstanceInScene: true, })
            {
                return true;
            }
        ...
        {
            using (new OnlyAssetsEditableScope(gameObjectStatus))
            {
                EditorGUI.BeginChangeCheck();
                ...
                var nodeIssues = sync.TryGetComponent<CoherenceNode>(out _) ? CoherenceNodeValidator.Validate(sync) : CoherenceNodeValidator.Issue.None;

                if (nodeIssues.HasFlag(CoherenceNodeValidator.Issue.InvalidInterpolationLocation))
                {
                    CoherenceNodeValidator.DrawFixInterpolationLocationHelpBox(serializedObject, gameObjectStatus, GUIContents.InvalidInterpolationLocationMessage);
                }
                else if (interpolationLocationProperty.intValue == 0)
                {
                ...
                CoherenceNodeValidator.HandleDrawFixRigidbodyUpdateModeHelpBox(nodeIssues, serializedObject, gameObjectStatus, GUIContents.InvalidRigidbodyUpdateModeMessage);
            }
        }
        ...
        private void DrawSynchronizationChannelID()
        {
            _ = EditorGUILayout.PropertyField(synchronizationChannelProperty, GUIContents.Channels.synchronizationChannel);
        }
        ...
        internal static void DrawFixHelpBox(SerializedObject serializedObject, GameObjectStatus status, string message, GUIContent buttonLabel, Action<SerializedObject> fixSync, MessageType messageType = MessageType.Warning)
        {
            EditorGUILayout.BeginVertical(GUI.skin.box);
            ...
            if (status is { IsRootOfPrefabStageHierarchy: true, PrefabStageMode: PrefabStageMode.InContext })
            {
                if (GUILayout.Button(GUIContents.openPrefabInIsolation, GUIStyles.bigButton))
                {
                    var targets = serializedObject.targetObjects;
                    var firstGameObject = targets.Select(o => o is Component c ? c.gameObject : null).First(g => g);
                    var firstGameObjectStatus = new GameObjectStatus(firstGameObject);
                    PrefabUtils.OpenInIsolation(firstGameObject, firstGameObjectStatus, true);
                }
            }
            else if (status.IsInstanceInScene || status.IsNestedInstanceInsideAnotherPrefab)
            {
                if (PrefabUtility.IsPartOfAnyPrefab(serializedObject.targetObject)
                    && GUILayout.Button(GUIContents.selectPrefabAsset, GUIStyles.bigButton))
                {
                    var targets = serializedObject.targetObjects;
                    var firstGameObject = targets.Select(o => o is Component c ? c.gameObject : null).First(g => g);
                    var firstGameObjectStatus = new GameObjectStatus(firstGameObject);
            ...
            else if (status.IsAsset || status is { IsRootOfPrefabStageHierarchy: true, PrefabStageMode: PrefabStageMode.InIsolation })
            {
                GUI.enabled = guiWasEnabled;
                if (GUILayout.Button(buttonLabel, ContentUtils.GUIStyles.bigButton))
                {
                    var targets = serializedObject.targetObjects;
                    if (serializedObject.targetObject is CoherenceSync)
                    {
                        fixSync(serializedObject);
                        serializedObject.ApplyModifiedProperties();
                    }
                    else
                    {
                        using var coherenceSyncSerializedObject = new SerializedObject(GetCoherenceSyncTargets());
                        fixSync(coherenceSyncSerializedObject);
                        coherenceSyncSerializedObject.ApplyModifiedProperties();

                        Object[] GetCoherenceSyncTargets()
                            => targets.Select(x => x && x is Component c && c.gameObject.TryGetComponent(out CoherenceSync s)
                                ? (Object)s
                                : null).Where(s => s).ToArray();
                    }

                    foreach (var assetPath in targets.Select(x => x is Component c ? AssetDatabase.GetAssetPath(c.gameObject) : null).Where(s => !string.IsNullOrEmpty(s)))
                    {
        ...
        internal static void DrawHelpBox(string message, MessageType messageType = MessageType.Warning)
        {
            EditorGUILayout.BeginVertical(GUI.skin.box);
            var guiWasEnabled = GUI.enabled;
            GUI.enabled = true;
