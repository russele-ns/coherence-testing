    ...
    public class INetworkObjectDrawer : PropertyDrawer
    {
...
#endif
        private static readonly Dictionary<Type, Type[]> objectProviderTypes = new();
        internal static readonly Dictionary<string, GUIContent> typeDisplayNames = new();
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(controlRect, displayName, FocusType.Keyboard, CoherenceHubLayout.Styles.PopupNonFixedHeight))
            {
                ...
                var typesPopup = new TypesPopup(objectProviderTypes[interfaceType], interfaceType, property, config);
        ...
        public static Type[] GatherObjectProviders(Type type)
        {
            ...
            var providerTypes = TypeCache.GetTypesDerivedFrom(type).Where(t => t.IsClass && !Attribute.IsDefined(t, typeof(ExcludeFromDropdownAttribute))).ToArray();

            foreach (var providerType in providerTypes)
            {
                typeDisplayNames[providerType.FullName] = DisplayNames.Get(providerType);
            }
