...
namespace Coherence.Toolkit
{
    ...
    [HelpURL("https://docs.coherence.io/v/2.0/manual/components/coherence-node")]
    [CoherenceDocumentation(DocumentationKeys.CoherenceNode)]
    public class CoherenceNode : MonoBehaviour
    {
        const int PATH_MAX_LENGTH = 64;
        ...
        private void DidSendConnectedEntity(CoherenceSync newConnectedEntity)
        {
            ...
            if (path.Length > PATH_MAX_LENGTH)
            {
                logger.Warning(Warning.ToolkitNodePathTooLong,
                    ("gameObject", gameObject.name), ("maxPathSize", PATH_MAX_LENGTH));
        ...
        {
            if (pathDirtyCounter != lastAppliedPathDirtyCounter)
            {
