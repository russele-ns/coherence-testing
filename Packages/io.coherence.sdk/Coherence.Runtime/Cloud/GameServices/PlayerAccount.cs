...
namespace Coherence.Cloud
{
    ...
    public sealed class PlayerAccount : IDisposable, IEquatable<PlayerAccount>
    {
        ...
        [MaybeNull] private static Logger logger;
        private static Logger Logger => logger ??= Log.GetLogger<PlayerAccount>();
        ...
        public static PlayerAccount Main
        {
            ...
            private set
            {
                ...
                {
                    Logger.Error(Error.ToolkitPlayerAccountError, ex.ToString());
                }
        ...
        internal static Action<Task<LoginResult>> OnLoginAttemptCompleted(TaskCompletionSource<PlayerAccount> taskCompletionSource, CloudService services, CancellationToken cancellationToken) => task =>
        {
            ...
            {
                services.Dispose();
                var error = new LoginError(ErrorType.InternalException, LoginErrorType.InternalException, Error.RuntimeInternalException, $"{nameof(OnLoginAttemptCompleted)} result was passed a task that is not completed yet.");
                taskCompletionSource.TrySetException(error);
                CoherenceCloud.RaiseOnLoggingInFailed(new(LoginErrorType.InternalException, error.ToString(), (Error.RuntimeInternalException)));
                return;
            }
            ...
            {
                services.Dispose();
                var loginError = new LoginError(loginResult.ErrorType, loginResult.LoginErrorType, loginResult.Error, loginResult.ErrorMessage);
                taskCompletionSource.TrySetException(loginError);
                CoherenceCloud.RaiseOnLoggingInFailed(new(loginResult.LoginErrorType, loginError.ToString(), (Error.RuntimeInternalException), hasBeenObserved: true));
            }
        };
