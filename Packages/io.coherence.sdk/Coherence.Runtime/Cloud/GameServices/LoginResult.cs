...
namespace Coherence.Cloud
{
    ...
    public record LoginResult
    {
        ...
        internal static LoginResult Success(PlayerAccount playerAccount, LoginResponse response) => new(Result.Success, playerAccount, response, null);
        internal static LoginResult Failure(Result type, LoginError error) => new(type, null, null, error);

        private LoginResult(Result type, [MaybeNull] PlayerAccount playerAccount, LoginResponse? response, LoginError error)
        {
            ...
            Username = response?.Username ?? playerAccount?.Username ?? "";
            SessionToken = response?.SessionToken ?? playerAccount?.SessionToken ?? "";
        }
