...
namespace Coherence.Cloud
{
    ...
    public class HttpServer
    {
        ...
        private static readonly LazyLogger logger = Log.GetLazyLogger<HttpServer>();
        private int port;
        ...
        private async Task RouteRequest(HttpListenerContext ctx)
        {
            ...
            {
                await fn(ctx, this, logger.Logger);
            }
            else
            {
                await WriteResponse(ctx, HttpStatusCode.NotFound, logger.Logger);
            }
        }
