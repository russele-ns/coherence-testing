...
namespace Coherence.Runtime
{
    ...
    {
        private static readonly LazyLogger logger = Log.GetLazyLogger(typeof(Request));
                        ...
                        case UnityWebRequest.Result.DataProcessingError:
                            ...
                            if (!string.IsNullOrEmpty(response.Result) &&
                                RequestException.TryParse(response.Result, (int)req.responseCode,
                                    out RequestException exception, logger.Logger))
                            {
        ...
        internal static async Task<string> ExecuteCustomAsync(string endpoint, string path, string method, string body)
        {
                    ...
                    switch (req.result)
                    {
                        ...
                        case UnityWebRequest.Result.DataProcessingError:
                            ...
                            if (!string.IsNullOrEmpty(response) &&
                                RequestException.TryParse(response, (int)req.responseCode, out RequestException exception, logger.Logger))
                            {
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(error) && RequestException.TryParse(error, (int)response.StatusCode, out RequestException exception, logger.Logger))
                    {
                        throw exception;
                    }
           ...
           Dictionary<string, string> headers, IRuntimeSettings settings, string sessionToken, string requestId, Action<RequestResponse<string>> callback)
        {
            ...
            req.SetRequestHeader(CloudService.ClientVersionHeader, "unity-sdk-v" + settings.SdkVersion);
            req.SetRequestHeader(CloudService.RSVersionHeader, settings.RsVersion);
                ...
                try
                {
                    switch (req.result)
                    {
                        ...
                        case UnityWebRequest.Result.DataProcessingError:
                            ...
                            if (string.IsNullOrEmpty(response.Result) ||
                                !RequestException.TryParse(response.Result, (int)req.responseCode, out RequestException exception, logger.Logger))
                            {
            ...
            Dictionary<string, string> headers, IRuntimeSettings settings, string sessionToken, string requestId, bool silenceWarning = false)
        {
            ...
            req.SetRequestHeader("X-Coherence-Client", "unity-sdk-v" + settings.SdkVersion);
            req.SetRequestHeader("X-Coherence-Engine", settings.RsVersion);
            req.SetRequestHeader(CloudService.RequestIDHeader, requestId);
            ...
            request.Headers.Add("X-Coherence-Client", "unity-sdk-v" + settings.SdkVersion);
            request.Headers.Add("X-Coherence-Engine", settings.RsVersion);
