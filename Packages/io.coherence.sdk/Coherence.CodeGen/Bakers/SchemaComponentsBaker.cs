...
namespace Coherence.CodeGen
{
    ...
    public class SchemaComponentsBaker : IBaker
    {
        public BakeResult Bake(CodeGenData codeGenData)
        {
            ...
            var bakedComponents = new List<BakedComponent>();
            ...
            foreach (var componentDef in componentDefinitions)
            {
                ...
                WriteComponentDefinition(bakedComponents, codeGenData, componentDef, archetypes);
            }

            BakeArchetypeComponents(bakedComponents, codeGenData, componentsDictionary);

            var scribanOptions = GetOptions(codeGenData);
            scribanOptions.Model[0]["components"] = bakedComponents;
            scribanOptions.Model[0]["extended_def"] = codeGenData.ExtendedDefinition;

            var renderResult =
                scribanWriter.Render(scribanOptions, System.IO.Path.Combine(codeGenData.OutputDirectory, "Interop"), "baked_components");
        ...
        private static void BakeArchetypeComponents(List<BakedComponent> bakedComponents, CodeGenData codeGenData,
            ...
            foreach (var archetype in codeGenData.SchemaDefinition.ArchetypeDefinitions)
            {
                foreach (var lod in archetype.lods)
                {
                    foreach (var component in lod.items)
                    {
                        ...
                        WriteComponentDefinition(bakedComponents, codeGenData, componentDef, emptyArchetypesList);
                    }
                }
            }
        }
        ...
        private static void WriteComponentDefinition(List<BakedComponent> bakedComponents, CodeGenData codeGenData, ComponentDefinition componentDef,
            List<ArchetypeDefinition> archetypes)
        {
            var hasRefFields = componentDef.members.Any(b => b.typeName.Equals("Entity"));

            var newBakedComponent = new BakedComponent
            {
                componentDefinition = componentDef,
                archetypes = archetypes,
                hasRefFields = hasRefFields,
                fieldsWithSimFrames = componentDef.members.Where(m => m.overrides.TryGetValue("sim-frames", out var val) ? val == "true" : false).ToList()
            };

            bakedComponents.Add(newBakedComponent);
        }
