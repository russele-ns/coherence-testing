...
namespace Coherence.Toolkit.Tests
{
    ...
    public class FloatingOriginManagerTests : CoherenceTest
    {
        ...
        public override void SetUp()
        {
            ...
            _ = clientMock.Setup(c => c.IsConnected()).Returns(true);

            var clientMockFloatingOrigin = Vector3d.zero;
            _ = clientMock.Setup(c => c.SetFloatingOrigin(It.IsAny<Vector3d>())).Callback<Vector3d>(fo => clientMockFloatingOrigin = fo);
            _ = clientMock.Setup(c => c.GetFloatingOrigin()).Returns(() => clientMockFloatingOrigin);

            entitiesManagerMock = new Mock<IEntitiesManager>(MockBehavior.Strict);
            _ = entitiesManagerMock.Setup(e => e.NetworkEntities).Returns(new List<NetworkEntityState>());

            foManager = new FloatingOriginManager(clientMock.Object, entitiesManagerMock.Object, logger);

            entityIDGenerator = new EntityIDGenerator(Entity.ClientInitialIndex, Entity.MaxRelativeID, false, logger);
        ...
        {
            _ = clientMock.Setup(c => c.IsConnected()).Returns(false);
        ...
        public void SetFloatingOrigin_WhenImpreciseRange_ShouldWarnOrThrow(bool useTranslate, double foX, double foY, double foZ, bool shouldWarn, bool shouldThrow)
        {
            ...
            if (shouldThrow)
            {
                _ = Assert.Throws<ArgumentOutOfRangeException>(() => SetFloatingOrigin(fo, useTranslate));
                Assert_FONotShifted();
                Assert.That(foManager.GetFloatingOrigin(), Is.EqualTo(Vector3d.zero));
            }
            else
        ...
        public void SetFloatingOrigin_ShouldShiftNetworkEntities(bool useTranslate)
        {
            ...
            _ = SetFloatingOrigin(oldFo, useTranslate);
            ...
            _ = entitiesManagerMock.Setup(e => e.NetworkEntities).Returns(new[] { entity1.State, entity2.State, entity3.State });

            _ = SetFloatingOrigin(newFo, useTranslate);
        ...
        private class EntityMock
        {
            ...
            {
                _ = Sync.Setup(s => s.coherencePosition).Returns(() => Position);
                _ = Sync.Setup(s => s.ShouldShift()).Returns(() => ShouldShift);
                _ = Sync.Setup(s => s.ShiftOrigin(It.IsAny<Vector3d>()))
                    .Callback<Vector3d>(d => Position -= d.ToUnityVector3())
                    .Returns(true);
                _ = Sync.Setup(s => s.Updater).Returns(Updater.Object);
                _ = Sync.Setup(s => s.OnFloatingOriginShifted).Returns((a, b) => FloatingOriginShiftedArgs.Enqueue((a, b)));
                _ = Sync.Setup(s => s.HasParentWithCoherenceSync).Returns(() => IsParented);

                _ = Updater.Setup(u => u.TryFlushPosition(It.IsAny<bool>())).Returns(true);
