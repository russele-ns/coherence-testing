...
namespace Coherence.Toolkit.Tests.Relay
{
    ...
    public class CoherenceRelayManagerTests : CoherenceTest
    {
        ...
        [TestCase(false)]
        [TestCase(true)]
        [Description("Tests that Flush is called after Update during relay manager update cycle.")]
        public void TestFlushCalledAfterUpdate(bool withException)
        {
            if (withException)
            {
                relayMock.Setup(r => r.Flush()).Throws(new Exception());
            }
            ...
            relayMock.Verify(r => r.Update(), Times.Once);
            relayMock.Verify(r => r.Flush(), Times.Once);

            if (withException)
            {
                Assert.That(logger.GetCountForErrorID(Error.ToolkitRelayFlushException), Is.EqualTo(1));
            }
        }
    }
}
