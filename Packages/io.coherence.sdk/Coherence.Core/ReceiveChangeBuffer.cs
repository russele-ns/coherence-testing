...
namespace Coherence.Core
{
    ...
    internal class ReceiveChangeBuffer
    {
        ...
        public void TakeCommands(List<IEntityMessage> buffer)
        {
            TakeMessages(buffer, commands);
        }

        public void TakeInputs(List<IEntityMessage> buffer)
        {
            TakeMessages(buffer, inputs);
        }

        private void TakeMessages(
            List<IEntityMessage> buffer,
            List<ExpirableMessage> source)
        {
            if (source.Count == 0)
            {
                return;
            ...
            for (int i = source.Count - 1; i >= 0; i--)
            {
                ...
                if (messageResolver.IsResolvable(message))
                {
                    buffer.Add(message);
                    source.RemoveAt(i);
                    continue;
