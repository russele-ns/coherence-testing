 ...
 prefsFilePath ?? Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Prefs", "prefs.bin"); // Based on the exec path
        ...
        public void Save()
        {
            ...
            catch (Exception ex)
            {
                ...
                long streamSize = prefsBinary.Count;
                try
                {
                    if (File.Exists(prefsFilePath))
                    {
                        var info = new FileInfo(prefsFilePath);
                        currentFileSize = info.Length;
                    }
                }
                catch { /* ignore secondary errors */ }
                throw new Exception($"Failed to save prefs. Current file size: {currentFileSize} bytes, stream size: {streamSize} bytes.", ex);
            }
        }
